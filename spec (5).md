...
- **STT (speech-to-text) — Tier 1 languages (`en-IN`, `hi-IN`, `kn-IN`):** Zoho
  Catalyst *Zia* — primary (Section 6.4a). Sarvam *Saarika* — backup. Gemini → Groq
  Whisper → OpenAI Whisper below that.
...
5. **Each day is a 3-activity session — Speak → Game → Quiz — resumable at the exact
   ...
   - **Speak:** free tier — record a prompt sentence, get transcribed, get
     ...
     encouragement), hear the correction spoken back. **Premium — a short turn-based
     roleplay conversation** (Section 6.9b): the AI tutor replies in character each
     turn, correcting naturally as it goes, up to a per-day turn cap.
...
- **`/practice/:languageCode`** — the 3-activity session UI: a stepper (Speak → Game
  ...
  returns, never at the start unless nothing has been done yet.
...
|---|---|---|---|---|---|
| English (India) — Tier 1 | `en-IN` | **Zia (Zoho Catalyst)** | Sarvam → Gemini → Groq Whisper → OpenAI Whisper | Sarvam Bulbul v3 (stream) | OpenAI TTS |
| Hindi — Tier 1 | `hi-IN` | **Zia (Zoho Catalyst)** | Sarvam → Gemini → Groq Whisper → OpenAI Whisper | Sarvam Bulbul v3 (stream) | — |
| Kannada — Tier 1 | `kn-IN` | **Zia (Zoho Catalyst)** | Sarvam → Gemini → Groq Whisper → OpenAI Whisper | Sarvam Bulbul v3 (stream) | — |
...
### 6.4 Sarvam key rotation for speech
...
### 6.4a Zoho Catalyst OAuth providers — Zia (STT) and QuickML/GLM-4.7-Flash (LLM)
...
- **Phase 3 — Speech input:** Zia primary for Tier-1 languages, Sarvam primary for
  the rest, full fallback chain per 6.2. **Verify:** real Kannada/Hindi/English audio
  transcribes on Zia; force it down and confirm Sarvam picks up; confirm a Tamil
  recording correctly skips Zia entirely and starts at Sarvam.
...
- **OAuth redirect URIs** (Google Cloud Console, Zoho API Console) must be updated to
  the deployed backend URL before Phase 1's Google/Zoho sign-in will work in
  production — a local `localhost` redirect URI does not carry over automatically.
- **Post-deploy checklist:** every prior version's checklist, plus — sign in via
  Google, sign in via Zoho, complete a real Speak→Game→Quiz session, confirm it
...
tracks consistency independent of roadmap progress. Behind it, Tier-1 STT runs through Zoho's own Zia model
first, the LLM chain now includes Zoho's own hosted GLM-4.7-Flash alongside five other
...
4. **ZeptoMail —** separate product, zoho.com/zeptomail. Sign up, add and verify a
   sending domain (DNS records they'll give you), create a Mail Agent, generate a
   Send Mail Token → `ZEPTOMAIL_SEND_TOKEN`. This must be done before Phase 1's OTP
   email can be tested at all.
